...
from libery import Library as lib
import pandas as pd
from humen import Client as c
from humen import User as u
from libery import Library as lib
...
class LibraryDataManager:
    ...
    def login_user(self, name, password):
        encrypted_password = hashlib.sha256(password.encode()).hexdigest()
        #return self.users.get(name) == encrypted_password
        return True
    ...
    def borrow_book(self, user_name, book_name):
        if book_name in self.books and self.books[book_name] > 0:
            self.books[book_name] -= 1
            self.borrowed_books.append({"name": user_name, "book": book_name})
            return True
        return False

    def return_book(self, user_name, book_name):
        for record in self.borrowed_books:
            if record["name"] == user_name and record["book"] == book_name:
                self.borrowed_books.remove(record)
                self.books[book_name] = self.books.get(book_name, 0) + 1
                return True
        return False

    def add_to_waitlist(self, user_name, book_name):
        self.waitlist.append({"name": user_name, "book": book_name})

    def add_client(self, client_name):
        self.clients.append(client_name)

    def get_top5_most_pupular(self):
        lib.get_top5_most_pupular()
...
class LibraryUI:
    ...
    def show_login_screen(self, login_callback):
        ...
        tk.Button(frame, text="התחבר", command=on_login, font=("Arial", 14),
                  bg="#4CAF50", fg="white", activebackground="#45a049", padx=20, pady=10).pack(pady=20)
...
class LibraryController:
    ...
    def load_main_menu(self):
        menu_options = [
            ...
            ("הוספת לקוח", self.add_client_screen),
